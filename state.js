...
sanitizeSave();
G.deckId = S.deckId || 'basics';
G.diff = S.diff || 'all';
if (!['learn', 'quiz', 'trivia', 'debug', 'build', 'order', 'speed', 'algo'].includes(S.mode))
    S.mode = 'learn';
G.mode = S.mode;
...
function load() {
    let s = null;
    try {
        s = JSON.parse(localStorage.getItem('gopher-arena-v4') || 'null') || JSON.parse(localStorage.getItem('gopher-arena-v3') || 'null');
    }
    catch (e) {
        s = null;
    }
    ...
    if (typeof raw.v === 'number' && raw.v > SAVE_VERSION) {
        ...
        try {
            localStorage.setItem('gopher-arena-v4-future', JSON.stringify(s));
        }
        catch (_) { }
        return null;
    }
    ...
    if (s) {
        ...
        s.lastDay = today;
        try {
            localStorage.setItem('gopher-arena-v4', JSON.stringify(s));
        }
        catch (_) { }
    }
    return s;
...
export function sanitizeSave() {
    ...
    S.wrong = num(S.wrong);
    S.quest = Math.min(8, num(S.quest));
    S.hearts = num(S.hearts, 3);
    S.bestSpeed = num(S.bestSpeed);
    S.trivBest = num(S.trivBest);
    S.trivDone = num(S.trivDone);
    S.trivOrder = (S.trivOrder || []).filter((x) => typeof x === 'string');
    S.trivIdx = num(S.trivIdx);
    S.trivScore = num(S.trivScore);
    S.dbgOrder = (S.dbgOrder || []).filter((x) => typeof x === 'string');
    S.dbgIdx = num(S.dbgIdx);
    S.dbgSolved = num(S.dbgSolved);
    S.dbgDone = num(S.dbgDone);
    ...
    // on import/upgrade.
    if (S._questPaid && !S.questPaidDay && S.quest >= 8 && S.questDay)
...
function migrateSaveVersion(s) {
    ...
    if (typeof s['v'] !== 'number' || s['v'] < SAVE_VERSION) {
        if (typeof s['lastActiveDay'] !== 'string' || !s['lastActiveDay']) {
            ...
            if (typeof s['dayStreak'] !== 'number' || !s['dayStreak'])
                s['dayStreak'] = s['lastActiveDay'] ? 1 : 0;
            dirty = true;
        }
        if (typeof s['questPaidDay'] !== 'string') {
            s['questPaidDay'] = '';
            dirty = true;
        }
        ...
        if (!Array.isArray(s['trivOrder'])) {
            s['trivOrder'] = [];
            dirty = true;
        }
        if (typeof s['trivIdx'] !== 'number') {
            s['trivIdx'] = 0;
            dirty = true;
        }
        if (typeof s['trivScore'] !== 'number') {
            s['trivScore'] = 0;
            dirty = true;
        }
        if (!Array.isArray(s['dbgOrder'])) {
            s['dbgOrder'] = [];
            dirty = true;
        }
        if (typeof s['dbgIdx'] !== 'number') {
            s['dbgIdx'] = 0;
            dirty = true;
        }
        s['v'] = SAVE_VERSION;
...
function reconcileContent(m, s = S) {
    let dirty = false;
    const keep = new Set(m.cards);
    const stores = [s.mastered, s.starred, s.score, s.mistakes];
    ...
    if (s.seenIds)
        for (const k of Object.keys(s.seenIds)) {
            if (!seenKeep.has(k)) {
                delete s.seenIds[k];
                dirty = true;
            }
    ...
    for (const [done, steps, counts] of flows) {
        ...
        if (steps)
            for (const k of Object.keys(steps)) {
                ...
                if (typeof steps[k] !== 'number' || steps[k] > max) {
                    steps[k] = Math.min(Math.max(0, num(steps[k])), max);
                    dirty = true;
                }
                if (done && done[k]) {
                    delete steps[k];
...
export function ensureToday() {
    const today = dayStr();
    if (S.questDay === today && S.lastDay === today)
        return false;
    if (S.questDay && S.questDay !== today) {
...
export function touchActive() {
    ...
    S.dayStreak = nx.dayStreak;
    S.lastActiveDay = nx.lastActiveDay;
...
export function replaceState(next) {
    ...
    S.questHistory = S.questHistory || {};
    S.questPaidDay = S.questPaidDay || '';
    if (!S.lastActiveDay && S.lastDay)
        S.lastActiveDay = S.lastDay;
    G.deckId = S.deckId || 'basics';
    G.diff = S.diff || 'all';
    G.mode = S.mode;
    G.combo = 1;
