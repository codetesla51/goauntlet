...
export function interleave(weak, rest, rand = Math.random) {
    weak = [...weak].sort(() => rand() - .5);
    rest = [...rest].sort(() => rand() - .5);
    const out = [];
    let wi = 0, ri = 0;
...
export function normOut(s) { return (s || '').replace(/\r\n/g, '\n').trim(); }
/**
 ...
 */
export function insertionIndex(mids, y) {
 ...
 */
export function movePlacedAt(arr, from, to) {
    const out = [...arr];
...
export function freshFirst(items, key, seen) {
...
export function mergeTriviaOrder(savedIds, allIds, rand = Math.random) {
    ...
    for (const id of (savedIds || [])) {
        if (allIds.indexOf(id) !== -1 && !seen.has(id)) {
            seen.add(id);
            valid.push(id);
        }
    }
...
export function shiftDayKey(day, delta) {
...
export function isYesterday(last, today) {
...
export function nextStreak(dayStreak, lastActiveDay, today) {
    if (lastActiveDay === today)
        return { dayStreak, lastActiveDay, changed: false };
    if (isYesterday(lastActiveDay, today))
        return { dayStreak: (dayStreak || 0) + 1, lastActiveDay: today, changed: true };
...
export function displayStreak(dayStreak, lastActiveDay, today) {
    if (lastActiveDay === today)
        return dayStreak || 0;
    if (isYesterday(lastActiveDay, today))
        return dayStreak || 0;
...
export function migrateStores(stores, cards) {
    const byQ = {};
    const byId = {};
