...
import type { TraceShow, SchedShow } from './content.js';
...
export function mountOptions(
  box: HTMLElement,
  labels: string[],
  onPick: (i: number, btn: HTMLButtonElement) => void,
  o?: { btnClass?: string; ctaFirst?: boolean; animate?: boolean; keyHint?: boolean },
): HTMLButtonElement[] {
  ...
  return labels.map((t, i) => {
    ...
    if (animate) { btn.style.animationDelay = (i * 0.06) + 's'; btn.classList.add('rise'); }
    box.appendChild(btn);
    return btn;
  });
}
...
export function markOptions(btns: HTMLButtonElement[], correctIdx: number, pickedIdx: number): void {
  btns.forEach((b, i) => {
...
export function mountCodeLines(
  box: HTMLElement,
  rows: Array<{ gutter: string; codeHtml: string; aria: string }>,
  onPick: (i: number, btn: HTMLButtonElement) => void,
  o?: { animate?: boolean },
): HTMLButtonElement[] {
  const animate = !o || o.animate !== false;
  ...
  return rows.map((r, i) => {
    ...
    if (animate) { b.style.animationDelay = (i * 0.04) + 's'; b.classList.add('rise'); }
    box.appendChild(b);
    return b;
  });
}

/* Pool → placed arranger: tap lines into order, drag to reorder, tap placed
   to send back. Covers lesson order steps, lesson build steps (decoys are
   just extra pool items) and Order mode — same gestures, different graders.
   Items are indexes into `lines`; the component owns the two arrays.
   Every move funnels through commit() (fresh paint); graders repaint
   directly via paint() to keep their green marks. */
export interface ArrangeHooks {
  poolEl: HTMLElement;
  placedEl: HTMLElement;
  undoBtn?: HTMLButtonElement | null;
  lines: string[];
  emptyText: string;
  emptyHtml?: string;
  poolGripHtml?: string;
  sortPoolOnRemove: boolean;
  isLocked: () => boolean;
  guardPlace?: () => boolean;
  goodPrefix: () => number;
  lockedMarks: () => boolean;
  isCorrectAt: (pos: number, item: number) => boolean;
  onInput: () => void;
  onPoolDrained?: () => void;
  pool: number[];
  placed?: number[];
}
export interface ArrangeCtl {
  pool: number[];
  placed: number[];
  paint(): void;
  reset(pool: number[]): void;
  undoLast(): void;
}
export function mountArrange(h: ArrangeHooks): ArrangeCtl {
  const grip = h.poolGripHtml || '::';
  const ctl: ArrangeCtl = {
    pool: [...h.pool],
    placed: h.placed ? [...h.placed] : [],
    paint,
    reset(pool: number[]) { ctl.pool = [...pool]; ctl.placed = []; commit(); },
    undoLast() {
      const li = ctl.placed.pop();
      if (li === undefined) return;
      ctl.pool.push(li);
      if (h.sortPoolOnRemove) ctl.pool.sort((a, b) => a - b);
      commit();
    },
  };
  ...
  let drag: { src: string; i: number } | null = null;
  let dropPos = -1;

  function commit(): void {
    h.onInput();
    if (!ctl.pool.length && !h.isLocked() && h.onPoolDrained) h.onPoolDrained();
    paint();
  }
  function tapPool(item: number): void {
    if (h.isLocked()) return;
    if (h.guardPlace && !h.guardPlace()) return;
    ctl.pool = ctl.pool.filter((x) => x !== item);
    ctl.placed.push(item);
    commit();
  }
  function tapPlaced(pos: number): void {
    if (h.isLocked() || pos < 0 || pos >= ctl.placed.length) return;
  ...
  function dropAt(pos: number): void {
    const d = drag;
    drag = null;
    if (!d || h.isLocked()) return;
    clearIndicator();
    dropPos = -1;
    if (d.src === 'pool') {
      if (h.guardPlace && !h.guardPlace()) return;
  ...
  function paint(): void {
    ...
    h.poolEl.ondrop = (e) => {
      e.preventDefault();
      if (!drag || drag.src === 'pool') { drag = null; return; }
    ...
    ctl.pool.forEach((li) => {
      ...
      b.ondragstart = (e) => { drag = { src: 'pool', i: li }; try { e.dataTransfer && e.dataTransfer.setData('text/plain', 'p' + li); } catch (_) {} };
    ...
    ctl.placed.forEach((li, pos) => {
      ...
      b.ondragstart = (e) => { drag = { src: 'placed', i: pos }; try { e.dataTransfer && e.dataTransfer.setData('text/plain', 's' + pos); } catch (_) {} };
    ...
    pl.ondragover = (e) => {
      ...
      const mids = rows.map((r) => { const b = (r as HTMLElement).getBoundingClientRect(); return b.top + b.height / 2; });
      dropPos = insertionIndex(mids, (e as DragEvent).clientY);
    ...
    if (h.undoBtn) h.undoBtn.disabled = !ctl.placed.length;
  }
  paint();
  return ctl;
}
...
export function mountTypeAnswer(box: HTMLElement, o: {
  answer: string; placeholder?: string;
  onPass: (why: string) => void; onFail: () => void; why: string;
}): void {
  box.innerHTML = '';
  ...
  (msg as HTMLElement).style.cssText = 'background:var(--surface-2);border:1px dashed var(--line-strong);border-radius:var(--r-sm);';
  const check = () => {
  ...
  btn.onclick = check;
  inp.onkeydown = (e) => { if ((e as KeyboardEvent).key === 'Enter') check(); };
...
export function mountTrace(box: HTMLElement, o: {
  title: string; show: TraceShow; onFinish: () => void;
}): void {
  box.innerHTML = '';
  ...
  const count = bar.querySelector('span:last-child') as HTMLElement | null;
  const lanes = document.createElement('div');
  lanes.className = 'viz-bricks';
  lanes.style.display = 'block';
  const note = document.createElement('p');
  note.className = 'viz-note';
  win.appendChild(bar); win.appendChild(lanes); win.appendChild(note);
  const row = document.createElement('div');
  ...
  function paint(): void {
    ...
    if (count) count.textContent = (idx + 1) + '/' + frames.length;
    ...
    f.lanes.forEach((lane) => {
      ...
      cells.style.display = 'flex'; cells.style.flexWrap = 'wrap'; cells.style.gap = '8px'; cells.style.marginTop = '4px';
      ...
      lr.appendChild(lab); lr.appendChild(cells);
      lanes.appendChild(lr);
    });
    (prev as HTMLButtonElement).disabled = idx <= 0;
  ...
  next.onclick = () => {
    if (idx < frames.length - 1) {
      idx++; paint();
      if (idx >= frames.length - 1 && !finished) { finished = true; o.onFinish(); }
    } else if (!finished) { finished = true; o.onFinish(); }
  };
  row.appendChild(prev); row.appendChild(replay); row.appendChild(next);
  box.appendChild(win); box.appendChild(row);
  paint();
}
...
export function mountScheduler(box: HTMLElement, o: {
  show: SchedShow; onPass: (msg: string) => void; onStuck: (msg: string) => void; onTick: () => void;
}): void {
  box.innerHTML = '';
  const show = o.show;
  const capOf = (ch: string) => Math.max(0, show.channels[ch] || 0);
  const slotOf = (ch: string) => Math.max(1, capOf(ch));
  let bufs: Record<string, string[]> = {};
  let vars: Record<string, string> = {};
  let log: string[] = [];
  let seenLogs: Record<string, boolean> = {};
  let finished = false;
  const fired: Record<number, boolean> = {};
  const reset = (keepSeen: boolean) => {
    Object.keys(fired).forEach((k) => delete fired[Number(k)]);
    bufs = {}; Object.keys(show.channels).forEach((c) => { bufs[c] = []; });
    vars = {}; log = [];
    if (!keepSeen) seenLogs = {};
    finished = false;
  };
  reset(false);
  ...
  body.appendChild(chanBox); body.appendChild(varLine);
  ...
  function runnable(opIdx: number): boolean {
    const op = show.ops[opIdx];
    if (op.send) {
      const b = bufs[op.send.ch] || [];
      if (b.length >= slotOf(op.send.ch)) return false;
    }
    if (op.recv) {
  ...
  function headOp(g: number): number {
    for (let i = 0; i < show.ops.length; i++) {
      if (show.ops[i].gor !== g || fired[i]) continue;
      return runnable(i) ? i : -1;
    }
    return -2;
  }
  function runOp(i: number): void {
    const op = show.ops[i];
    fired[i] = true;
    if (op.send) { (bufs[op.send.ch] = bufs[op.send.ch] || []).push(op.send.val); }
    if (op.recv) {
      const b = bufs[op.recv.ch] || [];
      vars[op.recv.into] = b.length ? b.shift() as string : '';
    }
    if (op.set) vars[op.set.k] = op.set.v;
    if (op.add) vars[op.add.k] = String((Number(vars[op.add.k] || '0')) + op.add.by);
    if (op.print) log.push(op.print.replace(/\{(\w+)\}/g, (_, k) => vars[k] || ''));
    o.onTick();
  }
  function allFired(): boolean { return show.ops.every((_, i) => fired[i]); }
  ...
  function key(l: string[]): string { return l.join('|'); }
  function judge(): void {
    if (finished) return;
    const goal = show.goal.type;
    if (allFired()) {
      if (goal === 'drain') { finished = true; o.onPass(show.success); return; }
      if (goal === 'deadlock') { o.onStuck('You drained every op — but the mission was to TRAP the deadlock. Reset and find the schedule where nobody can move.'); return; }
      if (goal === 'logs') {
        const k = key(log);
        const hit = (show.goal.anyOf || []).some((want) => key(want) === k);
        if (hit) seenLogs[k] = true;
        const need = show.goal.needAll ? (show.goal.anyOf || []).length : 1;
        if (Object.keys(seenLogs).length >= need) { finished = true; o.onPass(show.success); return; }
        o.onStuck(hit
    ...
    if (!anyRunnable()) {
      if (goal === 'deadlock') { finished = true; o.onPass(show.success); return; }
  ...
  function paint(): void {
    ...
    show.goroutines.forEach((g, gi) => {
      ...
      if (h === -2) {
        ...
        btn.disabled = true; btn.classList.add('dim');
      } else if (h === -1) {
        ...
        btn.onclick = () => { if (finished) return; runOp(h); paint(); judge(); };
      }
      gorBox.appendChild(btn);
    });
  }
  function countFired(g: number): number {
    let n = 0;
    show.ops.forEach((op, i) => { if (op.gor === g && fired[i]) n++; });
    return n;
  }
  foot.appendChild(resetBtn);
  box.appendChild(win); win.appendChild(body); win.appendChild(logPre);
  box.appendChild(gorBox); box.appendChild(foot);
  paint();
