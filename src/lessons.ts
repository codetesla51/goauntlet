...
import type { Lesson, LesStep } from './content.js';
...
let lesVizIdx = 0;
/* One guided-flow engine drives two collections: lessons (Learn tab) and
   projects (Build tab). flowKind+P select the collection, the save maps,
   and the visible player's element prefix. Set ONLY at flow entry points
   (pickers/startFlow); async callbacks re-anchor instead of trusting P. */
type FlowKind = 'lesson' | 'project' | 'algo';
let flowKind: FlowKind = 'lesson';
let P = 'les';
function FLOWS(): Lesson[] { return flowKind === 'lesson' ? LESSONS : flowKind === 'project' ? PROJECTS : ALGOS; }
function doneMap(): Record<string, boolean> { return flowKind === 'lesson' ? S.lessonsDone : flowKind === 'project' ? S.projectsDone : S.algosDone; }
function stepMap(): Record<string, number> { return flowKind === 'lesson' ? S.lessonStep : flowKind === 'project' ? S.projectStep : S.algoStep; }
function flowWord(): string { return flowKind === 'lesson' ? 'Lesson' : flowKind === 'project' ? 'Project' : 'Algorithm'; }
function zoneEl(which: 'zone' | 'picker' | 'player' | 'list'): HTMLElement {
...
let lesClaimed: Record<string, boolean> = {};
...
let lesOrdCtl: ArrangeCtl | null = null;
let lesBldCtl: ArrangeCtl | null = null;
// Leading correct run, painted green after a check so retry keeps what's right.
let lesOrdGood = 0;
let lesBldGood = 0;
let lesBldAll: string[] = [];
let lesSpotBtns: HTMLButtonElement[] = [];
/* Shuffle line indexes, guaranteeing a real shuffle (never pre-solved). */
function shuffledLines(n: number): number[] {
  const pool = Array.from({ length: n }, (_, i) => i).sort(() => Math.random() - .5);
  if (n > 1 && pool.every((v, i) => v === i)) pool.reverse();
  return pool;
}
function curLesson(): Lesson | null {
  for (const l of FLOWS()) if (l.id === lesId) return l;
  return null;
}
function curLesStep(): LesStep | null {
  const l = curLesson();
  if (!l || !l.steps.length) return null;
  return l.steps[Math.min(lesIdx, l.steps.length - 1)];
}
function renderLessonPicker(): void { renderFlowPicker('lesson'); }
function renderProjectPicker(): void { renderFlowPicker('project'); }
function renderAlgoPicker(): void { renderFlowPicker('algo'); }
function renderFlowPicker(kind: FlowKind): void {
  flowKind = kind; P = kind === 'lesson' ? 'les' : kind === 'project' ? 'pro' : 'alg';
  zoneEl('zone').classList.remove('hidden');
  zoneEl('player').classList.add('hidden');
  zoneEl('picker').classList.remove('hidden');
  const box = zoneEl('list'); box.innerHTML = '';
  if (!FLOWS().length) {
...
function startFlow(id: string): void {
  lesId = id;
  const l = curLesson(); if (!l) return;
...
function lesBack(): void {
  persistLesStep();
  renderFlowPicker(flowKind);
}
function persistLesStep(): void {
  if (lesId && !doneMap()[lesId]) { stepMap()[lesId] = lesIdx; save(); }
}
function claimLesXP(key: string, xp: number, coins: number): void {
  if (lesClaimed[key]) return;
  lesClaimed[key] = true;
  touchActive();
  const before = level();
  S.xp += xp; S.coins += coins;
  save(); updateHUD(); sfx.coin(); xpBurst('+' + xp + ' XP');
  if (level() > before) setTimeout(showLevelUp, 450);
}
function renderLesStep(): void {
  const l = curLesson(); const s = curLesStep();
  if (!l || !s) { renderFlowPicker(flowKind); return; }
  ...
  $(P + 'Stage').innerHTML = '';
  $(P + 'Verdict').classList.add('hidden'); $(P + 'Verdict').innerHTML = '';
  ...
  if (s.kind === 'visual' && s.viz) { lesVizIdx = 0; paintLesViz(); }
  if (s.kind === 'quiz' && s.quiz) paintLesQuiz();
...
function lesPredictPick(i: number): void {
  ...
  if (s.kind === 'order' && s.lines) {
    ...
    lesOrdCtl = mountArrange({
      poolEl: $(P + 'OrdPool'), placedEl: $(P + 'OrdPlaced'), undoBtn: $<HTMLButtonElement>(P + 'OrdUndo'),
  ...
  if (s.kind === 'spot' && s.code) paintLesSpot();
  ...
  if (s.kind === 'build' && s.lines) {
    ...
    lesBldCtl = mountArrange({
      poolEl: $(P + 'BldPool'), placedEl: $(P + 'BldPlaced'), undoBtn: $<HTMLButtonElement>(P + 'BldUndo'),
...
function syncLesNav(): void {
  ...
  ($<HTMLButtonElement>(P + 'PrevBtn')).disabled = lesIdx <= 0;
  const nx = $<HTMLButtonElement>(P + 'NextBtn');
...
function lesNext(): void {
  const l = curLesson(); const s = curLesStep();
  if (!l || !s) return;
  if (!lesStepDone) { lesExplainLock(); return; }
  if (s.kind === 'read') claimLesXP(flowKind + ':' + lesId + ':' + lesIdx, 10, 2);
  if (lesIdx >= l.steps.length - 1) { completeLesson(); return; }
  lesIdx++;
  renderLesStep();
  popLes();
}
function lesExplainLock(): void {
  const s = curLesStep();
  const k = s ? s.kind : '';
  if (k === 'run' || k === 'fix') {
    if (s && s.predict && !lesPredicted) { toast('Call it first — pick what the code prints. Run unlocks after.', 'crystal-ball'); pulse($(P + 'Predict')); }
    else { toast('Press Run and match the goal output exactly.', 'play'); pulse($(P + 'RunBtn')); }
  }
  else if (k === 'order') { toast('Place every line in order, then Check order.', 'list-numbers'); pulse($(P + 'OrderWrap')); }
  else if (k === 'build') { toast('Place every true line, then Check build.', 'hammer'); pulse($(P + 'BuildWrap')); }
  else if (k === 'spot') { toast('Tap the broken line first — wrong taps cost nothing.', 'hand-pointing'); pulse($(P + 'SpotWrap')); }
  else if (k === 'quiz') { toast('Pick an answer — wrong picks cost nothing.', 'question'); pulse($(P + 'QuizWrap')); }
  else if (k === 'type') { toast('Type the exact output, then Check — wrong tries cost nothing.', 'keyboard'); pulse($(P + 'Stage')); }
  else if (k === 'trace') { toast('Step through to the end — the last frame completes the step.', 'slideshow'); pulse($(P + 'Stage')); }
  else if (k === 'sched') { toast('Run every op toward the goal — Reset rewinds a bad schedule.', 'users-three'); pulse($(P + 'Stage')); }
  else toast('Finish this step first.', 'hand-pointing');
}
function lesPrev(): void {
  if (lesIdx <= 0) return;
  lesIdx--;
  renderLesStep();
  popLes();
}
function popLes(): void { const el = zoneEl('player'); el.classList.remove('pop-in'); void el.offsetWidth; el.classList.add('pop-in'); }
function completeLesson(): void {
  const l = curLesson(); if (!l) return;
  if (!doneMap()[lesId]) {
    doneMap()[lesId] = true; delete stepMap()[lesId];
    touchActive();
    bumpQuest(1);
    S.xp += 25; S.coins += 10; save(); updateHUD();
...
function lesHighlight(): void {
  ...
  const v = $<HTMLTextAreaElement>(P + 'Code').value;
  $(P + 'Hl').innerHTML = highlight(v) + '\n';
  lesSyncScroll();
}
function lesSyncScroll(): void {
  const ta = $<HTMLTextAreaElement>(P + 'Code');
  const pre = $(P + 'HlWrap');
  pre.scrollTop = ta.scrollTop; pre.scrollLeft = ta.scrollLeft;
}
function lesShowVerdict(ok: boolean, head: string, sub: string): void {
  const box = $(P + 'Verdict');
...
async function lesRun(): Promise<void> {
  const s = curLesStep();
  if (!s || (s.kind !== 'run' && s.kind !== 'fix')) return;
  ...
  const btn = $<HTMLButtonElement>(P + 'RunBtn'); btn.disabled = true;
  const code = $<HTMLTextAreaElement>(P + 'Code').value;
  ...
  try {
    ...
    const j = await res.json() as { errors?: string; output?: string };
    ...
    if (j.errors) {
      ...
      setText(P + 'Out', '$ go run main.go\n' + (j.output || '(no output)') + '\n— exit 0');
      if (got === want) lesStepComplete("It ran, and the output matches the goal. That wasn't luck — that was you.");
      else lesShowVerdict(false, "It compiled! But the output isn't the goal yet.", 'Compare them line by line. The goal box above shows exactly what success looks like.');
    }
  } catch (e) {
    if (!alive()) { done(); return; }
...
function lesStepComplete(msg: string): void {
  if (lesStepDone) return;
  lesStepDone = true;
...
function lesCopyRead(): void {
  const s = curLesStep();
  const code = s && s.code ? s.code : '';
  copyText(code).then(ok => { if (ok) toast('Code copied.', 'copy'); else showCopyFallback(code); });
}
function lesPlayground(): void {
  const ta = document.getElementById(P + 'Code') as HTMLTextAreaElement | null;
  const code = ta && ta.value ? ta.value : '';
  window.open('https://go.dev/play/', '_blank');
  copyText(code).then(ok => showPlayHandoff(code, ok));
}
function lesReadToPlayground(): void {
  const s = curLesStep();
  const code = s && s.code ? s.code : '';
  window.open('https://go.dev/play/', '_blank');
  copyText(code).then(ok => showPlayHandoff(code, ok));
...
function paintLesViz(): void {
  ...
  const box = $(P + 'VizBricks'); box.innerHTML = '';
  viz.nums.forEach((v, i) => {
    ...
    b.className = cls; b.textContent = String(v);
    const mk = document.createElement('span');
    mk.className = 'viz-ptr'; mk.textContent = ptrMid || '\u00a0';
    cell.appendChild(b); cell.appendChild(mk);
    box.appendChild(cell);
  });
  ($<HTMLButtonElement>(P + 'VizPrev')).disabled = lesVizIdx <= 0;
  ($<HTMLButtonElement>(P + 'VizNext')).disabled = lesVizIdx >= viz.frames.length - 1;
  ($(P + 'VizPrev') as HTMLButtonElement).onclick = () => { if (lesVizIdx > 0) { lesVizIdx--; paintLesViz(); } };
  ($(P + 'VizNext') as HTMLButtonElement).onclick = () => {
    if (lesVizIdx < viz.frames.length - 1) {
      lesVizIdx++; paintLesViz();
      if (lesVizIdx >= viz.frames.length - 1) lesStepComplete('You watched the whole hunt — now you think in halves.');
    }
  };
  ($(P + 'VizReplay') as HTMLButtonElement).onclick = () => { lesVizIdx = 0; paintLesViz(); };
}
function paintLesQuiz(): void {
  const s = curLesStep();
  if (!s || !s.quiz) return;
  $(P + 'QuizMsg').classList.add('hidden');
  mountOptions($(P + 'QuizOpts'), s.quiz.options, (i) => lesQuizPick(i),
    { btnClass: 'tokbtn pred-opt', animate: false });
}
function lesQuizPick(i: number): void {
  const s = curLesStep();
  if (!s || !s.quiz) return;
  const msg = $(P + 'QuizMsg');
  ...
  if (i === s.quiz.answer) {
    ...
    ($(P + 'QuizOpts') as HTMLElement).innerHTML = '';
...
function lesResetOrder(): void {
  const s = curLesStep(); if (!s || !s.lines || !lesOrdCtl) return;
  lesOrdCtl.reset(shuffledLines(s.lines.length));
}
function lesUndoOrd(): void { if (lesOrdCtl) lesOrdCtl.undoLast(); }
function lesUndoBld(): void { if (lesBldCtl) lesBldCtl.undoLast(); }
function lesCheckOrder(): void {
  const s = curLesStep(); if (!s || !s.lines || !lesOrdCtl) return;
...
function paintLesSpot(): void {
  const s = curLesStep();
  const lines = (s && s.code ? s.code : '').split('\n');
  lesSpotBtns = mountCodeLines($(P + 'SpotLines'),
    lines.map((ln, i) => ({ gutter: String(i + 1), codeHtml: highlight(ln), aria: 'Line ' + (i + 1) + ': ' + ln })),
    (i) => lesSpotPick(i));
}
function lesSpotPick(i: number): void {
  const s = curLesStep();
  if (!s || lesStepDone) return;
  if (i === s.bug) {
...
function lesResetBuild(): void {
  if (!lesBldCtl) return;
  lesBldCtl.reset(shuffledLines(lesBldAll.length));
}
function lesCheckBuild(): void {
  const s = curLesStep(); if (!s || !s.lines || !lesBldCtl) return;
  ...
  if (decoyAt !== -1) lesShowVerdict(false, 'Position ' + (decoyAt + 1) + ' of your build is an impostor — green lines are locked in.', 'One pool line looks right but breaks the program. Tap it to send it back, or Undo one step.');
  else if (placed.length !== n) lesShowVerdict(false, 'Not the whole program yet — green lines are locked in.', 'You placed ' + placed.length + ' of ' + n + ' needed lines.');
  else lesShowVerdict(false, 'Right lines, wrong order — green lines are locked in.', 'Position ' + (firstBad + 1) + ' is the first line out of place. What has to come before it?');
  sfx.bad();
}

export { renderLessonPicker, renderProjectPicker, renderAlgoPicker, startFlow, lesNext, lesHighlight, lesSyncScroll,
  lesBack, lesPrev, lesHint, lesCopyRead, lesReadToPlayground, lesRun, lesReset,
  lesPlayground, lesCheckOrder, lesResetOrder, lesCheckBuild, lesResetBuild,
  lesUndoOrd, lesUndoBld };
