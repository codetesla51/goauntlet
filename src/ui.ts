...
import type { Confetto } from './content.js';
...
function rankFor(l: number): string { let r: string = RANKS[0][1]; for(const [lv,name] of RANKS){ if(l>=lv) r=name; } return r; }
function needRevive(): boolean { if(S.hearts>0) return false; try{ const om=$<HTMLDialogElement>('overModal'); if(!om.open) om.showModal(); }catch(_){} return true; }
function buyShield(){
  if(S.shield) return toast('Shield already active — it forgives your next mistake.','shield-check');
  if(S.coins<50) return toast('Need 50 coins for a shield.','coins');
  S.coins-=50; S.shield=true; save(); updateHUD(); sfx.coin(); toast('Shield equipped.','shield-check');
}
function buyHeart(){ if(S.coins<40) return toast('Need 40 coins.','coins'); if(S.hearts>=5) return toast('Lives are already full.','heart'); S.coins-=40; S.hearts++; save(); updateHUD(); sfx.coin(); toast('Extra life acquired.','heart'); }
...
function unlock(id: string): void {
  if(S.ach[id]) return; S.ach[id]=true; save(); renderAch();
  const def=ACH_DEFS.find(x=>x.id===id);
  toast('Achievement unlocked: '+(def?def.name:id)+'.','trophy'); sfx.coin(); confetti(50);
}
function checkAch(){
  if(S.correct>=1) unlock('first');
  if(S.streak>=5) unlock('streak5');
  if(S.streak>=10) unlock('streak10');
  if(S.coins>=200) unlock('rich');
  if(Object.keys(S.mastered).length>=15) unlock('scholar');
}
function renderAch(){
...
function updateHUD(){
  ...
  setText('hudLevel', l); setText('hudLevelM', l);
  setText('hudXp', xpIn);
  $('xpBar').style.width=pct; $('xpBarM').style.width=pct;
  setText('hudRank', rankFor(l)); setText('rankTitle', rankFor(l).toUpperCase());
  ...
  if(S.quest>=8 && S.questPaidDay!==S.questDay){ S.questPaidDay=S.questDay; S._questPaid=true; S.xp+=150; S.coins+=30; save(); toast('Daily quest complete — +150 XP.','scroll'); confetti(100); }
  $('shieldLine').classList.toggle('hidden',!S.shield);
  const pips=$('comboPips'); pips.innerHTML='';
  for(let i=0;i<4;i++) pips.innerHTML+='<span class="h-3 flex-1 rounded-full" style="background:'+(i<G.combo?'var(--muted)':'var(--line)')+'"></span>';
...
function updateDeckProgress(){
  ...
  $('deckBar').style.width=pct+'%'; setText('deckPct', pct+'%');
}
function updateShop(): void {
  const cards = G.mode==='quiz'||G.mode==='speed';
  ($('shopShield') as HTMLButtonElement).disabled = G.mode==='trivia' || S.shield || S.coins<50;
  ($('shopHeart') as HTMLButtonElement).disabled = G.mode==='trivia' || S.coins<40 || S.hearts>=5;
  ($('shopSkip') as HTMLButtonElement).disabled = !cards || S.coins<30;
...
function openHelp(): void { $<HTMLDialogElement>('helpModal').showModal(); }
const cvs=$<HTMLCanvasElement>('confetti');
...
function confetti(n=80){
  ...
  void 0;
  for(let i=0;i<n;i++) parts.push({x:innerWidth/2+(Math.random()-.5)*260, y:innerHeight*0.32, vx:(Math.random()-.5)*11, vy:Math.random()*-9-2, g:.32, s:Math.random()*7+3, c:colors[i%colors.length], r:Math.random()*Math.PI, vr:(Math.random()-.5)*.3, life:90+Math.random()*40});
  if(!confettiRun){ confettiRun=true; requestAnimationFrame(tickConf); }
}
function tickConf(){
  ...
  if(parts.length) requestAnimationFrame(tickConf); else { confettiRun=false; ctx.clearRect(0,0,cvs.width,cvs.height); }
}
function syncDeckCard(): void {
...
function syncTabs(): void { document.querySelectorAll<HTMLElement>('.mode-tab').forEach(t=>t.classList.toggle('on', t.dataset.mode===G.mode)); }

export { level, rankFor, updateHUD, updateDeckProgress, renderAch, unlock, checkAch,
  buyShield, buyHeart, revive, updateShop, syncTabs, syncDeckCard, confetti,
  tryPlayground, openHelp, needRevive, quip, showLevelUp, ACH_DEFS };
/* Moved to owning module. */
function quip(): string { return QUIPS[Math.floor(Math.random()*QUIPS.length)] }
function showLevelUp(){
  setText('lvlNum', level());
  setText('lvlRank', rankFor(level()));
  setText('lvlBlurb', rankBlurb(level()));
  $<HTMLDialogElement>('levelModal').showModal(); sfx.level(); confetti(160);
}
...
export function exportProgress(): void {
  ...
  } catch (e) { toast('Export failed in this browser.', 'warning'); }
}
export function importProgressFile(input: HTMLInputElement): void {
  ...
  rd.onload = () => {
    try {
      ...
      if (!obj || typeof obj.xp !== 'number' || typeof obj.mastered !== 'object') throw new Error('bad file');
      if (typeof obj.v === 'number' && obj.v > SAVE_VERSION) { toast('That backup is from a NEWER app — open it there, or update this copy first.', 'warning'); return; }
      replaceState(obj);
      toast('Progress imported — reloading.', 'upload');
      setTimeout(() => location.reload(), 600);
    } catch (e) { toast('That file is not a progress backup.', 'warning'); }
  };
  rd.readAsText(f);
  input.value = '';
}
