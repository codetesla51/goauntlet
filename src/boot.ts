...
function goCards(){
  clearInterval(speedTimer); clearInterval(trivTimer);
  if(G.mode!=='quiz'&&G.mode!=='speed') G.mode='quiz';
  syncTabs(); persistUI(); syncDeckCard();
  $('flipScene').classList.remove('hidden'); $('deckProg').classList.remove('hidden');
...
function setMode(m: string): void {
  G.mode=m; clearInterval(speedTimer); clearInterval(trivTimer); resetReviewMode(); syncReviewBtn(); syncTabs(); persistUI(); syncDeckCard();
  syncZones();
  if(m==='learn') renderLessonPicker();
  else if(m==='speed') startSpeed();
  else if(m==='quiz') buildQueue();
  else if(m==='trivia') startTrivia();
  else if(m==='debug') startDebug();
  else if(m==='build') renderProjectPicker();
  else if(m==='order') startOrder();
  else if(m==='algo') renderAlgoPicker();
  else buildQueue();
  beep(600,.05);
  try{ $('modeRow').scrollIntoView({block:'nearest'}); }catch(_){}
  updateShop();
}
document.querySelectorAll<HTMLElement>('.mode-tab').forEach(t=>t.onclick=()=>setMode(t.dataset.mode ?? 'learn'));
...
for (const codeId of ['lesCode', 'proCode', 'algCode']) {
  ...
  ($(codeId) as HTMLTextAreaElement).addEventListener('keydown', (e) => {
...
} catch (e) { console.warn('[init] flow editor unavailable', e); }
async function boot(): Promise<void> {
  ...
  if (!DECKS.some(d => d.id === G.deckId)) G.deckId = DECKS[0].id;
  migrateProgress();
  ...
  updateHUD(); renderAch(); syncTabs(); updateShop(); syncDeckCard(); syncZones();
  if (G.mode === 'trivia') startTrivia();
  else if (G.mode === 'debug') startDebug();
  else if (G.mode === 'build') renderProjectPicker();
  else if (G.mode === 'order') startOrder();
  else if (G.mode === 'algo') renderAlgoPicker();
  else if (G.mode === 'speed') startSpeed();
  else if (G.mode === 'learn') renderLessonPicker();
  else buildQueue();
  if (warns.length) {
  ...
  try {
    ...
    try { showContentError('Startup failed: ' + (err instanceof Error ? err.message : String(err)) + ' — reload the page.'); } catch (_) {}
  }
}
...

{
  ...
  const ib = document.getElementById('impFile') as HTMLInputElement | null;
  if (ib) ib.addEventListener('change', () => importProgressFile(ib));
}
void boot();
...
import {
  ...
  ordNext, undoOrd, resetOrd, buySkip, dbgHint, startSpeed,
} from './quiz.js';
import {
