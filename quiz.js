...
import { DECKS, CARDS, TRIVIA, DEBUG, ORDER, LESSONS, PROJECTS, rankBlurb } from './content.js';
import { S, G, save, persistUI, touchActive, bumpQuest } from './state.js';
import { updateHUD, unlock, checkAch, level, rankFor, confetti, needRevive, syncTabs, quip, showLevelUp, updateDeckProgress } from './ui.js';
...
let trivTimer = 0;
const TIER = { rookie: 0, gopher: 1, boss: 2 };
let lastTier = -1, clearedPaid = false;
let attempted = {};
function isBoss(c, i) { return c.diff === 'boss' || (i % 10 === 9); }
function filteredCards() { return CARDS.filter(c => c.deck === G.deckId && (G.diff === 'all' || c.diff === G.diff)); }
function cur() { return queue[idx % queue.length]; }
function tierName(t) { return ['ROOKIE', 'GOPHER', 'BOSS'][t] || ''; }
function renderStages() {
    const el = $('stageDots');
    if (!el || !queue.length)
        return;
    const seen = {};
    queue.forEach(c => seen[TIER[c.diff] || 0] = true);
    ...
    if (mine > lastTier) {
        lastTier = mine;
        if (mine === 1) {
            toast('STAGE 2 — Gopher tier. Harder problems now.', 'lightning');
            sfx.coin();
        }
        if (mine === 2) {
            toast('FINAL STAGE — Boss problems. Triple stakes.', 'skull');
            sfx.coin();
            confetti(40);
        }
    }
    ...
    if (top === 2 && G.mode !== 'speed' && tierCleared(top) && !clearedPaid) {
        ...
        save();
        updateHUD();
        toast('Deck cleared! +25 XP bonus.', 'trophy');
        confetti(120);
        sfx.level();
    }
}
function tierCleared(t) { const cards = queue.filter(c => (TIER[c.diff] || 0) === t); return cards.length > 0 && cards.every(c => attempted[c.id]); }
...
function buildQueue() {
    ...
    if (reviewMode) {
        ...
        if (!pool.length) {
            toast('No mistakes yet — clean slate.', 'check-circle');
            reviewMode = false;
            syncReviewBtn();
            pool = filteredCards();
        }
        ...
        pool = filteredCards();
    if (!pool.length)
        pool = CARDS.filter(c => c.deck === G.deckId);
    if (!reviewMode) {
...
function renderOptions(c) {
    ...
    mountOptions($('quizOpts'), order, (i) => pickAnswer(norm(order[i]) === norm(c.a), $('quizOpts').children[i]));
}
/* Tap an option (or press 1-4 / A-D). That's it. */
function pickAnswer(ok, btn) {
    if (needRevive())
        return;
    if (locked)
        return;
    ...
    let correctIdx = -1;
    btns.forEach((b, i) => { if (norm(b.querySelector('span:nth-child(2)')?.textContent) === norm(c.a))
        correctIdx = i; });
    markOptions(btns, correctIdx, ok ? -1 : pickedIdx);
    const boss = isBoss(c, idx);
    if (ok && S.mistakes[c.id] > 0)
        S.mistakes[c.id]--;
    ...
    if (ok) {
        ...
        if (boss) {
            unlock('boss');
            toast('Boss defeated — triple XP.', 'skull');
            confetti(90);
        }
    }
    else
    ...
    if (G.mode === 'speed') {
        if (ok) {
            speedScore++;
            setText('speedScore', speedScore);
        }
        setTimeout(() => { if (reviewMode)
            buildQueue();
        else {
...
function award(ok, mult, forgiving) {
    mult = mult || 1;
    forgiving = !!forgiving;
    if (ok) {
        touchActive();
        ...
        const before = level();
        S.xp += gained;
        S.coins += 5 * mult + (G.combo > 1 ? G.combo * 2 : 0);
        S.streak++;
        S.best = Math.max(S.best, S.streak);
        pulse($('hudStreak').parentElement);
        if (S.streak === 3)
            toast('Warming up — 3 in a row.', 'fire');
        else if (S.streak === 8) {
            toast('Unstoppable — 8 streak.', 'fire');
            confetti(100);
        }
        else if (S.streak === 15) {
            toast('Gopher legend — 15 streak.', 'trophy');
            confetti(150);
        }
        S.correct++;
        S.seen++;
        ...
        if (S.streak > 0 && S.streak % 5 === 0)
            confetti(60);
        checkAch();
        if (rankFor(level()) !== rankFor(before))
            toast('New rank: ' + rankFor(level()) + ' — ' + rankBlurb(level()), 'trophy');
        if (level() > before)
    ...
    if (forgiving) {
        ...
        S.shield = false;
        toast('Shield absorbed the hit.', 'shield-check');
        sfx.coin();
    }
    else {
        ...
        G.combo = 1;
        sfx.bad();
        const hp = $('hudHearts').parentElement;
        if (hp) {
            hp.classList.remove('beat');
            void hp.offsetWidth;
            hp.classList.add('beat');
        }
        const fi = $('flipInner');
        if (fi) {
            fi.classList.remove('shake');
            void fi.offsetWidth;
            fi.classList.add('shake');
        }
        if (S.hearts <= 0) {
            save();
            updateHUD();
...
function cardMastery(id) {
    ...
    if (S.score[id] >= 2 && !S.mastered[id]) {
        S.mastered[id] = true;
        bumpQuest(1);
    }
}
function cardMiss(id) { S.score[id] = 0; S.mistakes[id] = (S.mistakes[id] || 0) + 1; if (S.mastered[id])
    delete S.mastered[id]; memorize(S.memory, id, false, Date.now()); }
...
function startTrivia() {
    ...
    if (savedValid.length) {
        ...
        trivOrder = mergedIds.map(id => TRIVIA.find(t => t.id === id));
...
function renderTrivia() {
    ...
    setText('trivQ', t.q);
    $('trivResult').classList.add('hidden');
    ...
    paintTriv();
    saveTriv();
    trivTimer = setInterval(() => { trivLeft -= 0.2; paintTriv(); if (trivLeft <= 0) {
        clearInterval(trivTimer);
        triviaTimeout();
    } }, 200);
}
...
function lockTriv() { trivLocked = true; clearInterval(trivTimer); [...$('trivOpts').children].forEach(b => b.disabled = true); }
function triviaPick(ok, btn) {
    if (needRevive())
        return;
    if (trivLocked)
        return;
    lockTriv();
    const t = trivOrder[trivIdx % trivOrder.length];
    const btns = [...$('trivOpts').children];
    const pickedIdx = btns.indexOf(btn);
    let correctIdx = -1;
    btns.forEach((b, i) => { if (norm(b.querySelector('span:nth-child(2)')?.textContent) === norm(t.a))
        correctIdx = i; });
    markOptions(btns, correctIdx, ok ? -1 : pickedIdx);
    markSeen(t.id);
    S.trivDone++;
    if (ok) {
        trivScore++;
        S.trivBest = Math.max(S.trivBest || 0, trivScore);
    }
    saveTriv();
    award(ok, 1, true);
    setText('trivScore', trivScore);
...
function triviaTimeout() {
    ...
    const t = trivOrder[trivIdx % trivOrder.length];
    markSeen(t.id);
    [...$('trivOpts').children].forEach(b => {
        if (norm(b.querySelector('span:nth-child(2)')?.textContent) === norm(t.a))
...
function trivResult(ok, head, explain) {
...
function startDebug() {
    ...
    if (savedValid.length) {
        ...
        dbgOrder = mergedIds.map(id => DEBUG.find(d => d.id === id));
...
function renderDebug() {
    ...
    $('dbgResult').classList.add('hidden');
    $('dbgResult').innerHTML = '';
...
function dbgVerdict(ok, head, explain) {
    const box = $('dbgResult');
    box.classList.remove('hidden');
    box.style.borderColor = ok ? '#3FA67A' : '#E04836';
    box.innerHTML = '<p class="font-extrabold flex items-center gap-2"><i class="ph-fill ' + (ok ? 'ph-check-circle' : 'ph-x-circle') + ' text-xl" style="color:' + (ok ? '#3FA67A' : '#E04836') + '"></i>' + head + '</p>'
...
function dbgHint() {
    if (dbgLocked)
        return;
    const d = dbgOrder[dbgIdx % dbgOrder.length];
    const kids = [...$('dbgLines').children];
    const innocents = kids.filter((b, j) => j !== d.bug && !b.disabled);
    if (innocents.length <= 2) {
        toast('Nothing left worth sniffing out.', 'lightbulb');
        return;
    }
    if (S.coins < 20) {
        toast('Not enough coins — answer cards to earn more.', 'coins');
        sfx.bad();
        return;
    }
    S.coins -= 20;
...
let ordOrder = [];
let ordCtl = null;
function updateOrdCount() { if (!ordCtl)
    return; const t = ordCtl.placed.length + ordCtl.pool.length; const el = $('ordCount'); if (el)
    el.textContent = ordCtl.placed.length + '/' + t + ' placed'; }
function startOrder() {
    if (!ORDER.length)
        return;
...
function renderOrder() {
    ...
    $('ordResult').classList.add('hidden');
    $('ordResult').innerHTML = '';
}
function undoOrd() { if (ordCtl)
    ordCtl.undoLast(); }
...
function resetOrd() { if (!ordCtl)
    return; ordTries = 2; ordGood = 0; renderOrder(); }
function gradeOrder() {
    if (!ordCtl)
        return;
    const o = ordOrder[ordIdx % ordOrder.length];
    ...
    if (ordTries <= 0) {
        ...
        award(false, 2, false);
        const plz = $('ordPlaced');
        plz.classList.remove('shake');
        void plz.offsetWidth;
        plz.classList.add('shake');
...
function verdict(boxId, ok, head, explain, next, extra) {
    const box = $(boxId);
    box.classList.remove('hidden');
    box.style.borderColor = ok ? '#3FA67A' : '#E04836';
    box.innerHTML = '<p class="font-extrabold flex items-center gap-2"><i class="ph-fill ' + (ok ? 'ph-check-circle' : 'ph-x-circle') + ' text-xl" style="color:' + (ok ? '#3FA67A' : '#E04836') + '"></i>' + head + '</p>'
...
function nextCard() { sfx.flip(); if (reviewMode) {
    buildQueue();
}
else {
    idx = (idx + 1) % queue.length;
...
function skipCard() { if (locked)
    return; idx = (idx + 1) % queue.length; S.streak = 0; G.combo = 1; renderCard(); updateHUD(); popCard('slide-l'); }
function popCard(dir) { const f = $('flipInner'); f.classList.remove('pop-in', 'slide-l', 'slide-r'); void f.offsetWidth; f.classList.add(dir || 'pop-in'); }
function copyCode() { copyText(cur().code || '').then(ok => { if (ok)
    toast('Code copied.', 'copy');
else
    showCopyFallback(cur().code || ''); }); }
function starCard() { const k = cur().id; S.starred[k] = !S.starred[k]; save(); renderCard(); toast(S.starred[k] ? 'Starred.' : 'Star removed.', 'star'); }
function useHint() {
    if (locked) {
        toast('You already answered this one.', 'lightbulb');
        return;
    }
    if (S.coins < 20) {
        toast('Not enough coins — answer cards to earn more.', 'coins');
...
function buySkip() { if (S.coins < 30)
    return toast('Need 30 coins.', 'coins'); S.coins -= 30; idx = (idx + 1) % queue.length; save(); updateHUD(); renderCard(); popCard('slide-l'); sfx.coin(); }
function resetReviewMode() { reviewMode = false; }
function toggleReview() {
    if (G.mode === 'learn')
        setMode('quiz');
    reviewMode = !reviewMode;
    if (reviewMode && !Object.keys(S.mistakes).some(k => S.mistakes[k] > 0)) {
        reviewMode = false;
        toast('No mistakes yet — clean slate.', 'check-circle');
        return;
    }
    ...
    buildQueue();
    if (reviewMode)
        toast('Review mode: your past mistakes first.', 'arrow-counter-clockwise');
}
function syncReviewBtn() { const b = $('reviewBtn'); if (b)
    b.classList.toggle('btn-primary', reviewMode); }
...
function startSpeed() {
    ...
    speedTimer = setInterval(() => {
        ...
        if (speedLeft <= 0) {
            ...
            if (speedScore > 0)
                touchActive();
            const bonus = speedScore * 10;
            S.xp += bonus;
            S.coins += speedScore * 3;
            if (speedScore >= 5)
                unlock('speed');
            save();
            updateHUD();
...
function paintSpeed() {
    ...
    const tb = $('timerBadge');
    tb.classList.remove('beat');
    void tb.offsetWidth;
    if (speedLeft <= 10 && speedLeft > 0)
        tb.classList.add('beat');
    $('speedBar').style.width = (Math.max(0, speedLeft) / 60 * 100) + '%';
    $('timerText').classList.toggle('blink', speedLeft <= 10 && speedLeft > 0);
}
document.querySelectorAll('#diffFilter button').forEach(b => b.onclick = () => {
    G.diff = b.dataset.diff ?? 'all';
    document.querySelectorAll('#diffFilter button').forEach(x => { x.classList.remove('btn-primary'); x.classList.add('btn-ghost'); x.style.borderColor = 'var(--line)'; });
...
document.addEventListener('keydown', e => {
    const kt = e.target;
    if (kt && kt.matches('input,textarea'))
        return;
    ...
    if (['1', '2', '3', '4', '5', '6', '7', '8', '9'].includes(e.key)) {
        const n = +e.key;
        if (G.mode === 'debug') {
            const lb = $('dbgLines').children[n - 1];
            if (lb)
                lb.click();
            return;
        }
        if (G.mode === 'order') {
            const pb = $('ordPool').children[n - 1];
            if (pb)
                pb.click();
            return;
        }
        ...
        const box = G.mode === 'trivia' ? $('trivOpts') : (cardMode ? $('quizOpts') : null);
        if (box) {
            const b = box.children['abcd'.indexOf(e.key.toLowerCase())];
            if (b)
                b.click();
        }
    }
    else if (e.key === 'ArrowRight') {
        if (cardMode && !$('resultBox').classList.contains('hidden'))
            nextCard();
        else if (G.mode === 'trivia' && !$('trivResult').classList.contains('hidden'))
            triviaNext();
        else if (G.mode === 'debug' && !$('dbgResult').classList.contains('hidden'))
            dbgNext();
        else if (G.mode === 'order' && !$('ordResult').classList.contains('hidden'))
            ordNext();
        else if (G.mode === 'learn' || G.mode === 'build' || G.mode === 'algo') {
            lesNext();
        }
    }
...
export { resetReviewMode, syncReviewBtn, renderDecks, cur, buildQueue, shuffleQueue, startSpeed, startTrivia, startDebug, startOrder, nextCard, prevCard, skipCard, copyCode, starCard, useHint, studyConfirm, flipCard, toggleReview, triviaNext, dbgNext, ordNext, undoOrd, resetOrd, buySkip, dbgHint, speedTimer, trivTimer };
